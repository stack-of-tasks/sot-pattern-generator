...
namespace dynamicgraph {
  namespace sot {
    ...
    void positionClipper(double x, double y,
      ...
      if(alpha < MIN_y/MAX_x) {
        ...
        double x_inter2 =
          (EDGE_POINT_y - delta1 * EDGE_POINT_x) / (alpha - delta1);
    ...
    starter( const int & timeCurr )
    {
      ...
      if(pgEntity) {
        ...
        for( std::deque< FootPrint >::const_iterator
               iter = footPrintList.begin();
              ...
              if (m_StepModificationMode == NextStepPgSot::ADDING_STEP)
                {
                  ...
                  cmdArgIn << cmdLine <<  " " << lastStep.x
                           << " " << lastStep.y << " " << lastStep.theta;
              ...
              else if (m_StepModificationMode == NextStepPgSot::CHANGING_STEP)
                {
                  if (m_NbOfFirstSteps<3)
                    ...
                    {
                      sotDEBUG(15) << "m_NextStepTime: "
                                   << m_NextStepTime << std::endl;
                          ...
                          if (lSupportFoot==1)
                            {
                              ...
                              double Next_xResult;
                              double Next_yResult;
                              positionClipper(NextFAPx, NextFAPy,
                                              Next_xResult, Next_yResult);
                              NextFAPx = cos(NextFAPtheta*PI/180)
                                * Next_xResult + sin(NextFAPtheta*PI/180)
                                * Next_yResult;
                              NextFAPy = -sin(NextFAPtheta*PI/180)
                                * Next_xResult +
